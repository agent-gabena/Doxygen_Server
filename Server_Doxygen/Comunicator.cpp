...
int Comunicator::self_addr(int port, std::string &err_way)
{
    ...
    throw ServerError("Failed to set socket options (SO_REUSEADDR)");
    }
    int b = bind(s,reinterpret_cast <const sockaddr* > (self_addr.get()),sizeof(sockaddr_in));//закрепляет сокет
    if(b == -1) {
        LogWriter::log(err_way, "Binding error (Критическая)");
...
int Comunicator::client_addr(int s, std::string &err_way)
{
    ...
    if(work_sock == -1) {
        ...
        LogWriter::log(err_way, "Ошибка подключения (Критическая)");
        return 1;
    } else {
                           ...
                           std::string &err_way) //аутентификация
{
    ...
    if(digest != new_msg) {
        ...
        close(work_sock);
        LogWriter::log(err_way, "Неправильный пароль (Не кртичиная)");
        throw ServerError("Неправильный пароль");
    } else {
